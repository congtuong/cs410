...
class ResidualBlock(nn.Module):
    ...
    def __init__(self, in_filter, out_filter, stride=1, activate_before_residual=False):
        ...
        self.conv1 = nn.Conv2d(in_filter, out_filter, kernel_size=3, 
        ...
        self.conv2 = nn.Conv2d(out_filter, out_filter, kernel_size=3,
...
class Model(nn.Module):
    ...
    def __init__(self, mode='eval', dataset='cifar10', train_batch_size=None):
        ...
        # Block 1
        self.layers.append(ResidualBlock(filters[0], filters[1], strides[0], activate_before_residual[0]))
        for i in range(1, 5):
            self.layers.append(ResidualBlock(filters[1], filters[1], 1, False))
        
        # Block 2
        self.layers.append(ResidualBlock(filters[1], filters[2], strides[1], activate_before_residual[1]))
        for i in range(1, 5):
            self.layers.append(ResidualBlock(filters[2], filters[2], 1, False))
        
        # Block 3
        self.layers.append(ResidualBlock(filters[2], filters[3], strides[2], activate_before_residual[2]))
        for i in range(1, 5):
        ...
        if self.mode == 'train' and train_batch_size is not None:
            self.register_parameter('pert', nn.Parameter(torch.zeros(train_batch_size, 3, 32, 32)))
        else:
            self.pert = None
